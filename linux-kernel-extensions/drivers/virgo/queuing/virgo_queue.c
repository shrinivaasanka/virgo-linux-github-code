	...
	*/
	if(clientsock != NULL )
	{
		...
		*/
		...
		/* 
			Invoking queue_func() directly instead of a kernel thread as clone_func() itself
...
int virgoqueue_sendto(struct socket* clsock)
{
	...
	*/
	if(clientsock != NULL)
	{
		...
		/*
		printk(KERN_INFO "virgoqueue_sendto(): kernel_recvmsg() returns len: %d\n",len);
		printk(KERN_INFO "virgoqueue_sendto(): kernel_recvmsg() returns in recv buffer: %s\n", buffer);
		*/
